...
// Import our custom Ollama client implementation
...
app.use((req, res, next) => {
  ...
  res.send = function(body) {
    console.log(`Response for ${req.method} ${req.url}: ${body.substring?.(0, 200) || body}`);
...
const LOG_LEVEL = process.env.LOG_LEVEL || 'debug'; // Set to debug by default
const logLevels = {
  debug: 0,
  info: 1,
  warn: 2,
...
// Initialize Ollama client with our custom implementation
...
${contextText}`;
    ...
    try {
      ...
      log('debug', `Result from Ollama: ${JSON.stringify(result).substring(0, 300)}...`);
      ...
      log('error', `Error in Ollama request: ${error.message}`);
